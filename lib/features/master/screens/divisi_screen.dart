...
class _DivisiScreenState extends State<DivisiScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
        (_) => context.read<MasterProvider>().fetchDivisi());
  }
    ...
    return Scaffold(
      ...
      body: Consumer<MasterProvider>(
        builder: (_, prov, __) {
          ...
          if (prov.divisiList.isEmpty)
            return EmptyState(
                message: 'Belum ada divisi',
                actionLabel: 'Tambah',
                onAction: () => _showForm());
              ...
              return Card(
                child: ListTile(
                  leading: CircleAvatar(
                    backgroundColor: AppColors.primary,
                    child: Text(d.divisiKode,
                        style: const TextStyle(
                            color: AppColors.white,
                            fontSize: 12,
                            fontWeight: FontWeight.w700)),
                  ),
                  title: Text(d.divisiNama,
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          await AppNotifier.showConfirm(
                            context,
                            title: 'Hapus Divisi',
                            message: 'Hapus divisi ${d.divisiNama}?',
                            onConfirm: () {
                              context
                                  .read<MasterProvider>()
                                  .deleteDivisi(d.divisiId);
                            },
                          );
                        },
                      ),
